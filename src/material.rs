...
use super::math::{Float3, ColorRGB, Ray, HitRecord};
use rand::{Rng};
...
impl Lambertian {
    pub fn new(a:ColorRGB) -> Lambertian {
        Lambertian { 
            albedo: a 
        }
    }
...
impl Scatter for Lambertian {
    fn scatter(&self, r_in: &Ray, rec: &HitRecord) -> Option<(ColorRGB, Ray)> {
        ...
        let mut scatter_dir = rec.normal + Float3::random_in_unit_sphere().normalized();
...
impl Metal { 
    pub fn new(a: ColorRGB, f: f64) -> Metal {
        Metal { 
            albedo: a,
            fuzz: f
        }
...
impl Scatter for Metal {
    fn scatter(&self, r_in: &Ray, rec: &HitRecord) -> Option<(ColorRGB, Ray)> {
        ...
        let scattered = Ray::new(rec.p, reflected + self.fuzz * Float3::random_in_unit_sphere());
...
impl Scatter for Dielectric { 
    ...
    Some((ColorRGB::new(1.0, 1.0, 1.0), scattered))
    }
}